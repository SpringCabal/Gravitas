...
function gadget:GetInfo()
  return {
	...
	author	= "Google Frog, adapted for BA by BrainDamage & then slightly modified for Gravitas by Bluestone",
	date	= "06/05/2013",
		...
		if validUnits[unitDefID] then
			...
			if targets then
				...
				if #targetList > 0 then
                    ...
                    if #targetList==1 then -- added for Gravitas, slightly hacky (it relies on no queues existing)
                        Spring.GiveOrderToUnit(unitID, CMD.ATTACK, {[1]=targetList[1].target}, {}, true) 
                    end
                end
			end
		end
...
local lineWidth = 1.4
local queueColour = {0.3, 0, 0.7, 0.7}
local commandColour = {0.3, 0, 0.7, 0.7}
...
function gadget:Initialize()
	...
	-- register cursor
	Spring.AssignMouseCursor("settarget", "cursorattack", false)
	--show the command in the queue
	Spring.SetCustomCommandDrawData(CMD_UNIT_SET_TARGET,"attack",queueColour,false)
	Spring.SetCustomCommandDrawData(CMD.ATTACK,"attack",queueColour,false)
end
